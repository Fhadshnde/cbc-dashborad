...
const SelectStoreForSurvey = () => {
  ...
  const fetchContracts = useCallback(async () => {
    ...
    setError('');
    try {
      const response = await axios.get('https://hawkama.cbc-api.app/api/merchant/contracts', {
        headers: {
      ...
      setError(err.response?.data?.message || err.message || 'فشل جلب العقود.');
  ...
  const handleSelectStore = (contractId, storeName) => {
  ...
  if (loading) {
    return (
      <div className="flex justify-center items-center h-screen bg-gray-100 rtl">
        <div className="text-center p-8 text-lg text-gray-600 animate-pulse">جاري تحميل المتاجر...</div>
      </div>
    );
  }
  ...
  return (
    <div className="m-4 sm:m-16 p-4 sm:p-6 bg-gray-50 min-h-screen text-right font-sans">
      <div className="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4 sm:gap-0">
        <h2 className="text-2xl font-bold text-gray-700">اختيار متجر لإنشاء استبيان</h2>
        <button 
      ...
      <div className="bg-white shadow rounded-lg p-4">
        {contracts.length > 0 ? (
              ...
              <tbody>
                {contracts.map((contract) => (
                  <tr key={contract._id} className="border-t hover:bg-gray-50">
                    ...
                    <td className="px-4 py-2 whitespace-nowrap">{contract.contractFullAddress || 'غير محدد'}</td>
                    <td className="px-4 py-2 whitespace-nowrap">
                      <button
                        onClick={() => handleSelectStore(contract._id, contract.storeName)}
          ...
          <p className="text-center text-gray-500 py-4">لا توجد عقود متاحة لإنشاء استبيان.</p>
        )}
      </div>
    </div>
  );
...
export default SelectStoreForSurvey;