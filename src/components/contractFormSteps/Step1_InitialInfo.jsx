...
const Step1_InitialInfo = ({ formData, handleChange, setFormData, setCurrentStep, contract, setError, setSuccess, setLoading, loading, getToken }) => {
  const calculateExpiryDate = (signingDateStr, period) => {
    ...
    if (isNaN(signingDate.getTime())) return '';
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (contract && contract._id) {
        ...
        res = await createInitialContractApi({ ...payload, status: "draft" }, getToken);
        setSuccess('Initial contract created successfully. You can now add details.');
      }
      setFormData(res.contract || res);
      setCurrentStep(2);
    } catch (err) {
  ...
  return (
    <form onSubmit={handleSubmit} className="p-6 rounded-lg">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
        <FloatingSelect label="نوع العقد *" name="contractType" value={formData.contractType || ''} onChange={handleLocalChange} required>
          <option value="">اختر نوع العقد</option>
          <option value="new">عقد جديد</option>
          <option value="renewal">تجديد عقد</option>
        </FloatingSelect>
        <FloatingInput label="رقم العقد *" placeholder="ادخل رقم العقد" name="contractNumber" value={formData.contractNumber || ''} onChange={handleLocalChange} required />
      </div>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
        <FloatingSelect label="مدة العقد *" name="contractPeriod" value={formData.contractPeriod || ''} onChange={handleLocalChange} required>
          <option value="">اختر مدة العقد</option>
          <option value="6 أشهر">6 أشهر</option>
          <option value="سنة">سنة</option>
          <option value="سنتين">سنتين</option>
        </FloatingSelect>
        <FloatingInput label="تاريخ التوقيع*" name="signingDate" value={formData.signingDate || ''} onChange={handleLocalChange} required type="date" />
      </div>
      <FloatingInput label="تاريخ الانتهاء *" name="expiryDate" value={formData.expiryDate || ''} onChange={handleLocalChange} required type="date" className="mb-6" />
      <FloatingInput label="اسم المتجر *" placeholder="ادخل اسم المتجر" name="storeName" value={formData.storeName || ''} onChange={handleLocalChange} required className="mb-6" />
      <FloatingSelect label="المحافظة *" name="contractGovernorate" value={formData.contractGovernorate || ''} onChange={handleLocalChange} required>
      ...
      <div className="mb-6 mt-4">
        ...
        <button type="button" onClick={handleGetLocation} className="bg-green-500 text-white px-4 py-2 rounded hover:bg-green-600 transition duration-300 ease-in-out" disabled={loading}>
          تحديد موقعي
        </button>
      ...
      <button type="submit" className="bg-[#25BC9D] text-white font-bold py-2 px-4 rounded-lg focus:outline-none focus:shadow-outline w-full transition-colors duration-300" disabled={loading}>
        المتابعة إلى تفاصيل الطرف الثاني
      </button>
    </form>
  );
};
