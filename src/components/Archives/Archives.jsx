...
const EXCEL_COLUMN_MAPPINGS = {
    ARABIC_HEADERS_SCHEMA: [
      { excelColumn: "اسم بالعربي", apiField: "name_ar", defaultValue: "غير معروف" },
      { excelColumn: "الاسم بالانكليزي", apiField: "name_en", defaultValue: "Unknown" },
      ...
      { excelColumn: "فئة الاشتراك", apiField: "cardCategory", defaultValue: "", type: "string" },
      { excelColumn: "المندوبة", apiField: "admin", defaultValue: "غير محدد" },
      { excelColumn: "التسلسل", apiField: "idOfcbc", defaultValue: (index) => `GENERATED-${Date.now()}-${index}`, type: "string" },
      { excelColumn: "تاريخ النفاذ", apiField: "date", defaultValue: null, type: "string" },
    ],
    EMPTY_HEADERS_SCHEMA: [
      ...
      { excelColumn: "الملاحظات", apiField: "notes", defaultValue: "" },
      { excelColumn: "__EMPTY_7", apiField: "cardCategory", defaultValue: "", type: "string" },
      { excelColumn: "__EMPTY_8", apiField: "ministry", defaultValue: "غير محددة" },
      { excelColumn: "رقم البطاقة", apiField: "idOfcbc", defaultValue: (index) => `GENERATED-${Date.now()}-${index}`, type: "string" },
      { excelColumn: "address", apiField: "address", defaultValue: "غير محدد" },
      { excelColumn: "__EMPTY_9", apiField: "date", defaultValue: null, type: "string" },
    ],
  };
...
const AccessArchive = () => {
  ...
  const exportToPdf = async (data, fileName = "Reports") => {
    ...
    data.forEach(report => {
      ...
      <td style="padding: 12px; border: 1px solid #e2e8f0;">${report.idOfcbc || ""}</td>
  ...
  const parseExcelData = (jsonData) => {
    ...
    const payloads = jsonData.map((row) => {
      const payload = {};
      activeMappingSchema.forEach(mapping => {
        let value = row[mapping.excelColumn];
        if (typeof value === 'string' && mapping.apiField === "cardCategory") {
          const lowerCaseCategory = value.toLowerCase();
          const now = new Date();
          let newExpirationDate = null;
          let card_id;

          if (lowerCaseCategory.includes("سنة")) {
            now.setFullYear(now.getFullYear() + 1);
            card_id = "1";
          } else if (lowerCaseCategory.includes("سنتين")) {
            now.setFullYear(now.getFullYear() + 2);
            card_id = "2";
          } else if (lowerCaseCategory.includes("افتراضي")) {
            now.setMonth(now.getMonth() + 6);
            card_id = "7";
          } else if (lowerCaseCategory.includes("مؤقتة")) {
            now.setMonth(now.getMonth() + 3);
            card_id = "13";
          }
          if (card_id) {
            newExpirationDate = `${now.getFullYear()}/${(now.getMonth() + 1).toString().padStart(2, "0")}`;
          }
          payload.cardCategory = {
            oneYear: lowerCaseCategory.includes("سنة") ? 1 : 0,
            twoYears: lowerCaseCategory.includes("سنتين") ? 1 : 0,
            virtual: lowerCaseCategory.includes("افتراضي") ? 1 : 0,
          };
          payload.date = newExpirationDate;
          payload.expire_period = newExpirationDate;
          payload.card_id = card_id;

        } else if (mapping.type === "string") {
          payload[mapping.apiField] = value ? String(value) : mapping.defaultValue;
        } else if (mapping.type === "number") {
          payload[mapping.apiField] = value ? Number(value) : mapping.defaultValue;
        } else if (value === undefined || value === null || (typeof value === 'string' && value.trim() === '')) {
          payload[mapping.apiField] = typeof mapping.defaultValue === 'function' ? mapping.defaultValue(row) : mapping.defaultValue;
        } else {
  ...
  const handleUpload = async () => {
    ...
    reader.onload = async (evt) => {
      try {
        ...
        const jsonData = XLSX.utils.sheet_to_json(worksheet, { header: 3, raw: false, dateNF:'YYYY/MM' });
        ...
        for (const payload of payloads) {
          try {
            await axios.post(`${API_URL}/archive`, payload, { headers: getAuthHeader() });
            successfulUploads++;
