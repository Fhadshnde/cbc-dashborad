...
const UserDetails = () => {
    ...
    const fetchUserReports = useCallback(async () => {
        ...
        try {
            ...
            const token = localStorage.getItem('token');
            if (!token) {
                setReportsError('لم يتم العثور على رمز المصادقة. يرجى تسجيل الدخول.');
            ...
            const response = await axios.get(`https://hawkama.cbc-api.app/api/reports/by-admin/${username}`, config);
    ...
    if (loading) {
        return (
            <div className="text-center p-5 font-sans rtl">
    ...
    // التعديل هنا في رابط الصورة:
    const userImageSrc = user.imageUrl
        ? `https://hawkama.cbc-api.app/api${user.imageUrl}`
        ...
        <div className="p-5 font-sans rtl text-right bg-gray-50 min-h-screen">
            <div className="bg-white rounded-lg shadow-md p-6 mb-6">
                ...
                <div className="tab-content">
                    ...
                    {activeTab === 'vacations' && <VacationsSection userId={user._id} vacations={user.vacations} />}
                </div>
            </div>
        </div>
    );
