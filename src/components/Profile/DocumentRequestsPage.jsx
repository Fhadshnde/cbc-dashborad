...
const DocumentRequestsPage = ({ setIsAuthenticated }) => {
  ...
  const fetchDocumentRequests = useCallback(async () => {
    ...
    try {
      ...
      console.error("خطأ في جلب طلبات المستندات:", err.response || err.message || err);
  ...
  }, [memoizedToken, memoizedUserData, navigate, setIsAuthenticated, fetchDocumentRequests]);
  ...
  const handleApproveDocument = async (userId, docId, status) => {
    ...
    try {
      ...
      fetchDocumentRequests();
      alert(`تم ${status === 'accepted' ? 'قبول' : 'رفض'} المستند.`);
    } catch (err) {
      console.error("خطأ في تحديث حالة المستند:", err.response || err.message || err);
  ...
  return (
    <div className="min-h-screen bg-gray-100 p-4 font-sans" dir="rtl">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-3xl font-bold text-gray-800">جميع طلبات المستندات</h1>
        <button
